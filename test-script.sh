...
function client {
    java -cp "./target/client.jar:./target/shared.jar" -Djava.security.policy=./target/classes/client.policy net.teamtrycatch.client.ATM localhost 7777 $@
}
...
client statement 300 01/01/2019 $(date +%d/%m/%Y)  # Create a date range from start of 2019 to now

echo
echo "Good sequence, switching accounts: login user 1, make deposit, login user 2, inquery"
